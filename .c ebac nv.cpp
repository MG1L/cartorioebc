...
int registro() // função respnsavel por cadastrar os usuários no sistema
{
	...
	file = fopen(arquivo, "a"); //coleta informação do usuário
	fprintf(file,nome); //salva o valor da variavel
	fclose(file); //fecha o arquivo
	
	file = fopen(arquivo, "a"); //coleta a informação do usuário
...
while(fgets(conteudo, 100, file) != NULL)
{
	...
	printf("%s", conteudo); //coletando informação do usuário
	printf("\n\n"); //coletando informação do usuário
}
...
int main()
	{
	int opcao=0; //Definindo variãveis
	int laco=1;
